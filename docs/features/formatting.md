# Text formatting

Inline marks: **bold**, *italic*, ~~strikethrough~~, `inline code`, and [links](links.md).

![Inline formatting](../images/formatting.png)

## How to apply

- **Type Markdown:** `**bold**`, `_italic_`, `~~strike~~`, `` `code` `` transform in place as
  you type the closing delimiter (see [input rules](../editing/input-rules.md)).
- **Toolbar / shortcuts:** select text and press **B**/**I** or
  <kbd>Ctrl/Cmd</kbd>+<kbd>B</kbd> / <kbd>Ctrl/Cmd</kbd>+<kbd>I</kbd>
  (see [keyboard shortcuts](../editing/keyboard-shortcuts.md)).

## Markdown

```markdown
**bold**, _italic_, ~~strikethrough~~, `inline code`, [a link](https://example.com)
```

Marks nest and combine (e.g. **bold _and italic_**), and always round-trip to canonical
Markdown.
